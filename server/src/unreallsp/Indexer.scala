...
class Indexer {
  ...
  private def collectFiles(dir: java.io.File): List[java.io.File] = {
    ...
    files.toList.flatMap { f =>
      if (f.isDirectory && !skipDirs.contains(f.getName)) {
        collectFiles(f)
      } else if (f.isFile && (isScalaFile(f.getName) || isJavaFile(f.getName))) {
        List(f)
      } else {
        Nil
      }
  ...
  private def isDefKeyword(tok: Token): Boolean = {
    tok match {
  ...
  private def readText(file: java.io.File): Option[String] = try Some(java.nio.file.Files.readString(file.toPath)) catch { case _: Exception => None }
          ...
          while (i < tokens.length) {
            ...
            tok match {
              ...
              case _ if isDefKeyword(tok) && depth <= 1 => {
                var j = i + 1
                while (j < tokens.length && !tokens(j).isInstanceOf[Token.Ident]) {
                  tokens(j) match {
  ...
  private def updateScalaFile(uri: String, text: String): Unit = {
    ...
    try {
      ...
      def traverse(tree: Tree): Unit = {
        tree match {
          case d: Defn.Class  => {
            addSymbol(d.name.value, d.name.pos)
  ...
  def reindexFile(uri: String, file: java.io.File): Unit = {
    ...
    val result = {
      if (isJavaFile(file.getName)) {
        scanJavaFile(file, Some(uri))
      } else {
        scanScalaFile(file, Some(uri))
      }
    }
    mergeResult(result)
  }
  ...
  def indexDirectory(dir: java.io.File): Unit = {
    ...
    try {
      val futures = files.map { f =>
        executor.submit(new Callable[FileResult] {
          def call() = {
  ...
  def updateFile(uri: String, text: String): Unit = {
    if (isJavaFile(uri)) {
      updateJavaFile(uri, text)
    } else {
      updateScalaFile(uri, text)
    }
  }
  ...
  def wordAtPosition(uri: String, line: Int, col: Int): Option[String] = {
    fileContents.get(uri).flatMap { text =>
      ...
      if (line < lines.length) {
        ...
        if (col <= l.length) {
          var start = col
          while (start > 0 && isIdentChar(l(start - 1))) {
            start -= 1
          }
          var end = col
          while (end < l.length && isIdentChar(l(end))) {
            end += 1
          }
          if (start < end) {
            Some(l.substring(start, end))
          } else {
            None
          }
        } else {
